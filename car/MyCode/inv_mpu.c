 ...
 */
int mpu_run_self_test(long *gyro, long *accel,int *flag)
{
#ifdef MPU6050
    ...
    if (ii == tries) {
		...
		accel_st[0] = accel_st[1] = accel_st[2] =0;
...
//返回值:0,正常
//    其他,失败
uint8_t run_self_test(int *flag)
{
    int result;
    //char test_packet[4] = {0};
    long gyro[3], accel[3];
    result = mpu_run_self_test(gyro, accel,flag);
    if (result == 0x3)
    {
				*flag +=5;
